...
const UserController = {
  getAllUsers: [authenticateUser, async (req, res) => {
    try {
      ...
      const users = await UserModel.find({ archive: false })
        .select('_id email first_name last_name address1 state country created_at')
        .skip(skip)
        .limit(limit);

      const count = await UserModel.countDocuments({ archive: false });
      res.status(200).json({
        users,
        count,
